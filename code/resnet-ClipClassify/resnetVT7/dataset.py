...
class VisualTactile(data.Dataset):
    ...
    def __getitem__(self,clip_index):
        ...
        clip, tactile, label = self.custom_getitem(front_vid_path, vid_frames, label, tac_path, pos_path, tac_frames)
        return clip, tactile, label, label_path
...
#         postion = pos_loader(pos_path, tac_frames)
        ...
        return video_to_tensor(clip),video_to_tensor(tactile), torch.from_numpy(label)

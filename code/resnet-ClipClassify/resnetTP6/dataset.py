...
def tp_fusedchannel_loader(tac_path, pos_path, frames):
    tac_image = tactile_loader(tac_path, frames)
    pos_image = pos_loader(pos_path, frames)
    tac_image = torch.from_numpy(tac_image)
    pos_image = torch.from_numpy(pos_image)
    return np.array(torch.cat((tac_image,pos_image),dim=-1))
...
class VisualTactile(data.Dataset):
    ...
    def __getitem__(self,clip_index):
        ...
        fused_tp, label = self.custom_getitem(label, tac_path, pos_path, tac_frames)
        return fused_tp, label, label_path

    def __len__(self):
        return len(self.clip_data)

    def custom_getitem(self, label, tac_path=None, pos_path=None, tac_frames=None):
        tp_fused = tp_fusedchannel_loader(tac_path,pos_path,tac_frames)
        return video_to_tensor(tp_fused), torch.from_numpy(label)
    ...
    def get_video_frames(self, video_index):
        ...
        for vid, tac, lab in zip(vid_frames, tac_frames, label):
            data.append(self.custom_getitem(lab, tac_path, pos_path, tac))
        return data, label_path

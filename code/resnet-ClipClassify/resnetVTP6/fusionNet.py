...
class FusionNet(nn.Module):
    ...
    def __init__(self, spatial_model, tp_model):
        ...
        #                                 nn.Linear(64, 1))
        self.fc = nn.Sequential(nn.Linear(512+64, 64, bias=True), nn.ReLU(inplace=True), nn.Dropout(p = 0.5),
                                            nn.Linear(64, 1))
    ...
    def forward(self, spatial_input, tp_input):
        ...
        # print('shape of concatenation= ', fused.shape)
        out = fused.view(fused.size(0), -1)
        # print('shape of flatten= ', fused.shape)
        out = self.fc(out)

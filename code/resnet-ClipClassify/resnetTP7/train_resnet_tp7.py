...
#         sm = self.freeze_network_layer(sm)
        tacNet = tactileNet.tactileNet()
        posNet = tactileNet.tactileNet()

        net = FusionNet(tacNet, posNet)
        if torch.cuda.is_available():
            net.cuda()
    ...
    def train(self):
        ...
        for epoch in range(epoch_num):
            ...
            for i,data in enumerate(self.dataloader):
                tac, pos, lab, path = data

                if torch.cuda.is_available():
                    tactile = Variable(tac.cuda())
                    position = Variable(pos.cuda())
                    label = Variable(lab.cuda())
                else:
                    tactile = Variable(tac)
                    position = Variable(pos)
                    label = Variable(lab)

                out = self.model(tactile.float(), position.float())
                out = out.squeeze(1)
            ...
            print("epoch {} :: time {}, total_train_time = {}".format(epoch,(time.time()-epoch_time)/60, train_time/60))

...
class Run_Model(object):
    ...
    def __init__(self, root, mode, test_dir, train_dir, save_model_withname=None,\
        ...
        self.dataloader = torch.utils.data.DataLoader(self.dataset, batch_size=self.batch_size, shuffle=True, num_workers=2, pin_memory=True)
...
#            pretrain = torch.load("../../../out/resnet/resnet-18-kinetics.pth", map_location="cpu")
    ...
    def train(self):
        ...
        for epoch in range(epoch_num):
            ...
            self.scheduler.step()
            print("epoch {} :: time {}".format(epoch,time.time()-epoch_time))
            if((epoch+1)%30 == 0):
    ...
    def test(self):
        ...
        for index in range(video_num):
            ...
            for i,dota in enumerate(packed_data):
                ...
                with open(self.save_error_withname, 'a') as file:
                    file.write('{}: Loss: {:.5f} and lr: {:.5f}.... Network output: {:.5f} and actual label: {} \n'.format(i,loss.item(),self.scheduler.get_lr()[0],out[0], label[0]))

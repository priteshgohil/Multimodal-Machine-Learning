"""
change log: resize frames to 160x120 instead of 320x240.
	    Randomcrop of 112x112 instead of 224x224
"""
...
#     print("duration",duration)
    ...
    while success:
        if((count%5)==0 and (len(images)<required_frames)):
            image = preprocess_image(image, (resize_W,resize_H))
            images.append(image)
            frames.append(count)
        success,image = vidcap.read()
        count += 1
...
def flow_loader(path, frames):
    ...
    for x,y in zip(images_x,images_y):
        ...
        img_x = preprocess_grayscale_image(img_x,(resize_W,resize_H))
        img_y = cv2.imread(path + y, cv2.IMREAD_GRAYSCALE)
        img_y = preprocess_grayscale_image(img_y,(resize_W,resize_H))
        images.append(np.stack((img_x,img_y)).transpose([1,2,0])) #output shape T x H x W x C
    if(len(images) != required_frames):
        raise ValueError("not enough flow frames {}, {}, {},{},,{},,{}".format(frame, path,frames,count,len(frames),images_x))
...
#         print("Samaples" , self.samples[0])
    ...
    def __getitem__(self, index):
        ...
        sample_f_video, selected_frames_f = vid_loader(f_video)
        sample_l_video, selected_frames_l = vid_loader(l_video)
        sample_tactile = tactile_loader(tactile,selected_frames_f)
        sample_pos = tactile_loader_old(pos)
        sample_target = target
        sample_f_flow = flow_loader(f_flow, selected_frames_f)
        sample_l_flow = flow_loader(l_flow, selected_frames_l)
        if self.transform is not None:

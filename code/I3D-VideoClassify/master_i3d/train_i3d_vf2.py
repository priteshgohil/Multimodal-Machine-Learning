...
from visual_tactile_dataset_vf2 import VisualTactile as Dataset
import cv2
...
def run(init_lr=0.01, max_steps=30, mode='rgb', root='/media/pritesh/Entertainment/Visual-Tactile_Dataset/dataset/',\
        train_split='train.txt', test_split='test.txt', batch_size=15, save_model='', save_filename=''):
    writer = tensorboardX.SummaryWriter(comment="V.testTrainv2.lr0.01.batch20.epoch30.resize112")
    # setup dataset
                ...
                if phase == 'val':
                    ...
                    if((steps+1)%30 == 0):
                       # torch.save(fusedNet.module.state_dict(), save_model+phase+str(steps).zfill(6)+'.pt')
                        save_checkpoint(fusedNet, optimizer, lr_sched, steps,save_model)

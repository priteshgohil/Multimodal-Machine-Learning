...
from hsr_dataset import *
from wrenchnet import WrenchNet
...
class Run_Model(object):
    ...
    def __init__(self, root, mode, test_dir, train_dir, save_model_withname=None,\
        ...
        train_dir_list = np.genfromtxt(self.train_dir, dtype=str)
        self.dataset = HSRDataset(self.root, train_dir_list, self.frames)
        ...
        test_dir_list = np.genfromtxt(self.test_dir, dtype=str)
        self.val_dataset = HSRDataset(self.root, test_dir_list, self.frames)
...
#         self.datasets = {'train': self.dataset, 'val': self.val_dataset}
    ...
    def load_model(self, checkpoint):
        spatialNet = resnet.resnet18(sample_size = 112, sample_duration = self.frames, num_classes = 400, shortcut_type='A')
        spatialNet = nn.DataParallel(spatialNet)
        if torch.cuda.is_available():
            pretrain = torch.load("../models/resnet-18-kinetics.pth")
            spatialNet.load_state_dict(pretrain['state_dict'])
        else:
...
#            pretrain = torch.load("../../../out/resnet/resnet-18-kinetics.pth", map_location="cpu")
            spatialNet.load_state_dict(pretrain['state_dict'])
        spatialNet = self.freeze_network_layer(spatialNet)
        ftNet = WrenchNet()

        net = FusionNet(spatialNet, ftNet)
        if torch.cuda.is_available():
            net.cuda()
    ...
    def train(self):
        ...
        for epoch in range(epoch_num):
            ...
            for i,data in enumerate(self.dataloader):
                # vid, tac, pos, lab, path = data
                if torch.cuda.is_available():
                    video = Variable(data['clip'].cuda())
                    force_torque = Variable(data['wrench'].cuda())
                    label = Variable(data['frame_label'].cuda())
                else:
                    video = Variable(data['clip'])
                    force_torque = Variable(data['wrench'])
                    label = Variable(data['frame_label'])

                out = self.model(video.float(), force_torque.float())
                out = out.squeeze(1)
            ...
            total_time += (time.time()-epoch_time)/60
    ...
    def test(self):
        ...
        for i,data in enumerate(self.val_dataset):
                ...
                force_torque = force_torque.unsqueeze(0)
                ...
                predicted_out = 1 if out[0]>0.5 else 0
